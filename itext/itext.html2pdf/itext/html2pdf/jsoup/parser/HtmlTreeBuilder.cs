...
namespace iText.Html2pdf.Jsoup.Parser {
    ...
    public class HtmlTreeBuilder : TreeBuilder {
        ...
        private iText.Html2pdf.Jsoup.Nodes.Element headElement;

        private FormElement formElement;

        private iText.Html2pdf.Jsoup.Nodes.Element contextElement;

        private List<iText.Html2pdf.Jsoup.Nodes.Element> formattingElements = new List<iText.Html2pdf.Jsoup.Nodes.Element
            >();
        ...
        internal virtual IList<Node> ParseFragment(String inputFragment, iText.Html2pdf.Jsoup.Nodes.Element context
            ...
            iText.Html2pdf.Jsoup.Nodes.Element root = null;
            if (context != null) {
                ...
                if (iText.Html2pdf.Jsoup.Helper.StringUtil.In(contextTag, "title", "textarea")) {
                    tokeniser.Transition(TokeniserState.Rcdata);
                }
                else {
                    if (iText.Html2pdf.Jsoup.Helper.StringUtil.In(contextTag, "iframe", "noembed", "noframes", "style", "xmp")
                        ) {
                ...
                // default
                root = new iText.Html2pdf.Jsoup.Nodes.Element(iText.Html2pdf.Jsoup.Parser.Tag.ValueOf("html"), baseUri);
                ...
                foreach (iText.Html2pdf.Jsoup.Nodes.Element parent in contextChain) {
        ...
        internal virtual void MaybeSetBaseUri(iText.Html2pdf.Jsoup.Nodes.Element @base) {
        ...
        internal virtual iText.Html2pdf.Jsoup.Nodes.Element Insert(Token.StartTag startTag) {
            // handle empty unknown tags
            // when the spec expects an empty tag, will directly hit insertEmpty, so won't generate this fake end tag.
            if (startTag.IsSelfClosing()) {
                iText.Html2pdf.Jsoup.Nodes.Element el = InsertEmpty(startTag);
                stack.Add(el);
                tokeniser.Transition(TokeniserState.Data);
            ...
            iText.Html2pdf.Jsoup.Nodes.Element el_1 = new iText.Html2pdf.Jsoup.Nodes.Element(iText.Html2pdf.Jsoup.Parser.Tag
                .ValueOf(startTag.Name()), baseUri, startTag.attributes);
            Insert(el_1);
            return el_1;
        }

        internal virtual iText.Html2pdf.Jsoup.Nodes.Element InsertStartTag(String startTagName) {
        ...
        internal virtual void Insert(iText.Html2pdf.Jsoup.Nodes.Element el) {
            InsertNode(el);
            stack.Add(el);
        }

        internal virtual iText.Html2pdf.Jsoup.Nodes.Element InsertEmpty(Token.StartTag startTag) {
            iText.Html2pdf.Jsoup.Parser.Tag tag = iText.Html2pdf.Jsoup.Parser.Tag.ValueOf(startTag.Name());
            iText.Html2pdf.Jsoup.Nodes.Element el = new iText.Html2pdf.Jsoup.Nodes.Element(tag, baseUri, startTag.attributes
                );
            InsertNode(el);
            if (startTag.IsSelfClosing()) {
                if (tag.IsKnownTag()) {
        ...
        private void InsertNode(Node node) {
            ...
            if (node is iText.Html2pdf.Jsoup.Nodes.Element && ((iText.Html2pdf.Jsoup.Nodes.Element)node).Tag().IsFormListed
                ()) {
                if (formElement != null) {
                    formElement.AddElement((iText.Html2pdf.Jsoup.Nodes.Element)node);
                }
            }
        }

        internal virtual iText.Html2pdf.Jsoup.Nodes.Element Pop() {
            int size = stack.Count;
            return stack.JRemoveAt(size - 1);
        }

        internal virtual void Push(iText.Html2pdf.Jsoup.Nodes.Element element) {
            stack.Add(element);
        }

        internal virtual List<iText.Html2pdf.Jsoup.Nodes.Element> GetStack() {
            return stack;
        }

        internal virtual bool OnStack(iText.Html2pdf.Jsoup.Nodes.Element el) {
            return IsElementInQueue(stack, el);
        }

        private bool IsElementInQueue(List<iText.Html2pdf.Jsoup.Nodes.Element> queue, iText.Html2pdf.Jsoup.Nodes.Element
             element) {
            for (int pos = queue.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element next = queue[pos];
                if (next == element) {
                    return true;
                }
            }
        ...
        internal virtual iText.Html2pdf.Jsoup.Nodes.Element GetFromStack(String elName) {
            for (int pos = stack.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element next = stack[pos];
        ...
        internal virtual bool RemoveFromStack(iText.Html2pdf.Jsoup.Nodes.Element el) {
            for (int pos = stack.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element next = stack[pos];
                if (next == el) {
                    stack.JRemoveAt(pos);
        ...
        internal virtual void PopStackToClose(params String[] elNames) {
            for (int pos = stack.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element next = stack[pos];
                stack.JRemoveAt(pos);
                if (iText.Html2pdf.Jsoup.Helper.StringUtil.In(next.NodeName(), elNames)) {
                    break;
                }
            }
        }
        ...
        private void ClearStackToContext(params String[] nodeNames) {
            for (int pos = stack.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element next = stack[pos];
                if (iText.Html2pdf.Jsoup.Helper.StringUtil.In(next.NodeName(), nodeNames) || next.NodeName().Equals("html"
                    )) {
                    break;
                }
                else {
        ...
        internal virtual iText.Html2pdf.Jsoup.Nodes.Element AboveOnStack(iText.Html2pdf.Jsoup.Nodes.Element el) {
        ...
        internal virtual void InsertOnStackAfter(iText.Html2pdf.Jsoup.Nodes.Element after, iText.Html2pdf.Jsoup.Nodes.Element
             @in) {
        ...
        internal virtual void ReplaceOnStack(iText.Html2pdf.Jsoup.Nodes.Element @out, iText.Html2pdf.Jsoup.Nodes.Element
             @in) {
            ReplaceInQueue(stack, @out, @in);
        }

        private void ReplaceInQueue(List<iText.Html2pdf.Jsoup.Nodes.Element> queue, iText.Html2pdf.Jsoup.Nodes.Element
             @out, iText.Html2pdf.Jsoup.Nodes.Element @in) {
        ...
        private bool InSpecificScope(String[] targetNames, String[] baseTypes, String[] extraTypes) {
            for (int pos = stack.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element el = stack[pos];
                String elName = el.NodeName();
                if (iText.Html2pdf.Jsoup.Helper.StringUtil.In(elName, targetNames)) {
                    return true;
                }
                if (iText.Html2pdf.Jsoup.Helper.StringUtil.In(elName, baseTypes)) {
                    return false;
                }
                if (extraTypes != null && iText.Html2pdf.Jsoup.Helper.StringUtil.In(elName, extraTypes)) {
                    return false;
                }
            }
        ...
        internal virtual bool InSelectScope(String targetName) {
            for (int pos = stack.Count - 1; pos >= 0; pos--) {
                ...
                if (!iText.Html2pdf.Jsoup.Helper.StringUtil.In(elName, TagSearchSelectScope)) {
                    // all elements except
                    return false;
                }
            }
        ...
        internal virtual void SetHeadElement(iText.Html2pdf.Jsoup.Nodes.Element headElement) {
            this.headElement = headElement;
        }

        internal virtual iText.Html2pdf.Jsoup.Nodes.Element GetHeadElement() {
            return headElement;
        }
        ...
        internal virtual void GenerateImpliedEndTags(String excludeTag) {
            while ((excludeTag != null && !CurrentElement().NodeName().Equals(excludeTag)) && iText.Html2pdf.Jsoup.Helper.StringUtil
        ...
        internal virtual bool IsSpecial(iText.Html2pdf.Jsoup.Nodes.Element el) {
            // todo: mathml's mi, mo, mn
            // todo: svg's foreigObject, desc, title
            String name = el.NodeName();
            return iText.Html2pdf.Jsoup.Helper.StringUtil.In(name, TagSearchSpecial);
        }

        internal virtual iText.Html2pdf.Jsoup.Nodes.Element LastFormattingElement() {
            return formattingElements.Count > 0 ? formattingElements[formattingElements.Count - 1] : null;
        }

        internal virtual iText.Html2pdf.Jsoup.Nodes.Element RemoveLastFormattingElement() {
        ...
        internal virtual void PushActiveFormattingElements(iText.Html2pdf.Jsoup.Nodes.Element @in) {
            int numSeen = 0;
            for (int pos = formattingElements.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element el = formattingElements[pos];
                if (el == null) {
                    // marker
                    break;
                }
        ...
        private bool IsSameFormattingElement(iText.Html2pdf.Jsoup.Nodes.Element a, iText.Html2pdf.Jsoup.Nodes.Element
             b) {
        ...
        internal virtual void ReconstructFormattingElements() {
            iText.Html2pdf.Jsoup.Nodes.Element last = LastFormattingElement();
            if (last == null || OnStack(last)) {
                return;
            }
            iText.Html2pdf.Jsoup.Nodes.Element entry = last;
            int size = formattingElements.Count;
            int pos = size - 1;
            bool skip = false;
            while (true) {
            ...
            while (true) {
                ...
                iText.Html2pdf.Jsoup.Nodes.Element newEl = InsertStartTag(entry.NodeName());
        ...
        internal virtual void ClearFormattingElementsToLastMarker() {
            while (!formattingElements.IsEmpty()) {
                iText.Html2pdf.Jsoup.Nodes.Element el = RemoveLastFormattingElement();
                if (el == null) {
                    break;
                }
            }
        ...
        internal virtual void RemoveFromActiveFormattingElements(iText.Html2pdf.Jsoup.Nodes.Element el) {
            for (int pos = formattingElements.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element next = formattingElements[pos];
                if (next == el) {
                    formattingElements.JRemoveAt(pos);
                    break;
                }
        ...
        internal virtual bool IsInActiveFormattingElements(iText.Html2pdf.Jsoup.Nodes.Element el) {
            return IsElementInQueue(formattingElements, el);
        }

        internal virtual iText.Html2pdf.Jsoup.Nodes.Element GetActiveFormattingElement(String nodeName) {
            for (int pos = formattingElements.Count - 1; pos >= 0; pos--) {
                iText.Html2pdf.Jsoup.Nodes.Element next = formattingElements[pos];
                if (next == null) {
                    // scope marker
        ...
        internal virtual void ReplaceActiveFormattingElement(iText.Html2pdf.Jsoup.Nodes.Element @out, iText.Html2pdf.Jsoup.Nodes.Element
             @in) {
        ...
        internal virtual void InsertInFosterParent(Node @in) {
            iText.Html2pdf.Jsoup.Nodes.Element fosterParent;
