...
namespace iText.Html2pdf.Jsoup.Nodes {
    ...
    ///     </summary>
    public class FormElement : iText.Html2pdf.Jsoup.Nodes.Element {
        private readonly iText.Html2pdf.Jsoup.Select.Elements elements = new iText.Html2pdf.Jsoup.Select.Elements(
            );
        ...
        public FormElement(iText.Html2pdf.Jsoup.Parser.Tag tag, String baseUri, Attributes attributes)
        ...
        public virtual iText.Html2pdf.Jsoup.Select.Elements Elements() {
            return elements;
        }
        ...
        public virtual iText.Html2pdf.Jsoup.Nodes.FormElement AddElement(iText.Html2pdf.Jsoup.Nodes.Element element
            ) {
        ...
        public virtual IList<KeyVal> FormData() {
            ...
            foreach (iText.Html2pdf.Jsoup.Nodes.Element el in elements) {
                ...
                if ("select".Equals(el.TagName())) {
                    iText.Html2pdf.Jsoup.Select.Elements options = el.Select("option[selected]");
                    bool set = false;
                    foreach (iText.Html2pdf.Jsoup.Nodes.Element option in options) {
                        data.Add(KeyVal.Create(name, option.Val()));
                        set = true;
                    }
                    if (!set) {
                        iText.Html2pdf.Jsoup.Nodes.Element option = el.Select("option").First();
                        if (option != null) {
                            data.Add(KeyVal.Create(name, option.Val()));
                        }
                    }
