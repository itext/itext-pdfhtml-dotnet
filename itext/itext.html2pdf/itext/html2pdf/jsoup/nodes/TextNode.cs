...
namespace iText.Html2pdf.Jsoup.Nodes {
    ...
    public class TextNode : Node {
        ...
        public virtual iText.Html2pdf.Jsoup.Nodes.TextNode Text(String text) {
            this.text = text;
            if (attributes != null) {
        ...
        public virtual bool IsBlank() {
            return iText.Html2pdf.Jsoup.Helper.StringUtil.IsBlank(GetWholeText());
        }
        ...
        public virtual iText.Html2pdf.Jsoup.Nodes.TextNode SplitText(int offset) {
            Validate.IsTrue(offset >= 0, "Split offset must be not be negative");
            Validate.IsTrue(offset < text.Length, "Split offset must not be greater than current text length");
            String head = GetWholeText().JSubstring(0, offset);
            String tail = GetWholeText().Substring(offset);
            Text(head);
            iText.Html2pdf.Jsoup.Nodes.TextNode tailNode = new iText.Html2pdf.Jsoup.Nodes.TextNode(tail, this.BaseUri(
                ));
            if (Parent() != null) {
        ...
        internal override void OuterHtmlHead(StringBuilder accum, int depth, OutputSettings @out) {
            if (@out.PrettyPrint() && ((SiblingIndex() == 0 && parentNode is iText.Html2pdf.Jsoup.Nodes.Element && ((iText.Html2pdf.Jsoup.Nodes.Element
                )parentNode).Tag().FormatAsBlock() && !IsBlank()) || (@out.Outline() && SiblingNodes().Count > 0 && !IsBlank
                ()))) {
                Indent(accum, depth, @out);
            }
            bool normaliseWhite = @out.PrettyPrint() && Parent() is iText.Html2pdf.Jsoup.Nodes.Element && !iText.Html2pdf.Jsoup.Nodes.Element
        ...
        public static iText.Html2pdf.Jsoup.Nodes.TextNode CreateFromEncoded(String encodedText, String baseUri) {
            String text = Entities.Unescape(encodedText);
            return new iText.Html2pdf.Jsoup.Nodes.TextNode(text, baseUri);
        }

        internal static String NormaliseWhitespace(String text) {
            text = iText.Html2pdf.Jsoup.Helper.StringUtil.NormaliseWhitespace(text);
            return text;
        }
        ...
        private void EnsureAttributes() {
            if (attributes == null) {
                attributes = new iText.Html2pdf.Jsoup.Nodes.Attributes();
                attributes.Put(TEXT_KEY, text);
            }
        }
        ...
        public override iText.Html2pdf.Jsoup.Nodes.Attributes Attributes() {
            EnsureAttributes();
